...
function ManagerialDetails(e) {
    ...
    if (msgErr == '') {
        ...
        $('#modalGerencial2 .modal-dialog .modal-body .body-excel').html(dataItem.IdEmpresa + ';' + dataItem.AnoMes);
...
function Creditos(objCred) {
    ...
    for (var i in objCred) {
        ...
        if (objCred[i].LinhasEspacadas == 1 || objCred[i].Totalizador != 0) {
            mClass = 'Ger-Lin';
            varHtml += '</tr>' + LinhaemBranco;
            TemLinha = 1;
        }
...
function Isentas(objIsen) {
    ...
    for (var i in objIsen) {
        ...
        if (objIsen[i].Linha.substring(0, 5) == 'Linha' && i < objIsen.length - 1)
            varHtml += LinhaemBranco;
    }
...
function BalanceDetails(e) {
    ...
    $('#modalSaldo .modal-dialog .modal-body .body-excel').html(dataItem.IdEmpresa + ';' + dataItem.AnoMes);
...
function CarregarMatrizes() {
    ...
    if (dsMatriz == undefined) {
        SShowModalAlert"Não foi possível obter as Matrizes.");
        return;
    }
...
function CarregarDetalhesCruzamento(tipoRegra, dsCruzamento) {
    ...
    switch (tipoRegra) {
        ...
        case "3":
            ...
            break;       
        default:
            SShowModalAlert"Tipo de Regra inválida!");
            break;
    }
}
...
function CarregarGridDetalhesNotasFiscais() {
    ...
    $("#gridDetalhesNotasFicais").kendoGrid({
        ...
        excelExport: function (e) {
            ...
            for (var rowIndex = 1; rowIndex < sheet.rows.length; rowIndex++) {
                ...
                if (row.type == "footer") {
                    ...
                    row.cells[6].value = kendo.toString(kendo.parseDate(row.cells[6].value, 'yyyy-MM-dd'), 'dd/MM/yyyy');
                    row.cells[7].value = kendo.toString(kendo.parseDate(row.cells[7].value, 'yyyy-MM-dd'), 'dd/MM/yyyy');
                    row.cells[8].value = parseFloat(row.cells[8].value).FormatarMoeda(2, "", ".", ",");
                    row.cells[9].value = parseFloat(row.cells[9].value).FormatarMoeda(2, "", ".", ",");
                    row.cells[10].value = parseFloat(row.cells[10].value).FormatarMoeda(2, "", ".", ",");
                }
        ...
        columns: [
            ...
            { field: "DataEmissao", title: "Data Emissão", footerTemplate: '<div><span style=\'float:right;\'>TOTAL GERAL:</span></div>', template: "#= kendo.toString(kendo.parseDate(DataEmissao, 'yyyy-MM-dd'), 'dd/MM/yyyy') #", width: "120px" },
            ...
            { field: "TotalItem", width: "120px", title: "Total Item", template: '#=TotalItem.FormatarMoeda(2, "", ".", ",") #', attributes: { style: "text-align:right;" }, footerTemplate: "<div style='float: right'>#= kendo.toString(sum, 'c2') #</div>" },
            { field: "ValorPIS", width: "120px", title: "Valor PIS", template: '#=ValorPIS.FormatarMoeda(2, "", ".", ",") #', attributes: { style: "text-align:right;" } , footerTemplate: "<div style='float: right'>#= kendo.toString(sum, 'c2') #</div>" },
            { field: "ValorCOFINS", width: "120px", title: "Valor COFINS", template: '#=ValorCOFINS.FormatarMoeda(2, "", ".", ",") #', attributes: { style: "text-align:right;" }, footerTemplate: "<div style='float: right'>#= kendo.toString(sum, 'c2') #</div>" }
        ]
    });
}
...
function CarregarGridDetalhesLancamentos() {
    ...
    $("#gridDetalhesLancamentos").kendoGrid({
        ...
        excelExport: function (e) {
            ...
            for (var rowIndex = 1; rowIndex < sheet.rows.length; rowIndex++) {
                ...
                if (row.type == "footer") {
                    ...
                    // Formatação de Data e Valor                
                    row.cells[3].value = kendo.toString(kendo.parseDate(row.cells[3].value, 'yyyy-MM-dd'), 'dd/MM/yyyy');                    
                    row.cells[4].value = parseFloat(row.cells[4].value).FormatarMoeda(2, "", ".", ",");
                }
        ...
        columns: [
            ...
            { field: "DataLancamento", title: "DataLancamento", footerTemplate: '<div><span style=\'float:right;\'>TOTAL GERAL:</span></div>', template: "#= kendo.toString(kendo.parseDate(DataLancamento, 'yyyy-MM-dd'), 'dd/MM/yyyy') #", width: "120px" },
            { field: "Valor", width: "120px", title: "Valor", template: '#=Valor.FormatarMoeda(2, "", ".", ",") #', attributes: { style: "text-align:right;" }, footerTemplate: "<div style='float: right'>#= kendo.toString(sum, 'c2') #</div>" }
        ]
    });
}
...
function CarregarGridDetalhesValoresDivergentes(ds) {
    ...
    $("#gridDetalhesValores").kendoGrid({
            ...
            for (var rowIndex = 1; rowIndex < sheet.rows.length; rowIndex++) {
                ...
                for (var rowIndex = 1; rowIndex < sheet.rows.length; rowIndex++) {
                    ...
                    if (row.type == "footer") {
                        ...
                        row.cells[9].value = parseFloat(row.cells[9].value).FormatarMoeda(2, "", ".", ",");
                        row.cells[10].value = parseFloat(row.cells[10].value).FormatarMoeda(2, "", ".", ",");
                        row.cells[11].value = parseFloat(row.cells[11].value).FormatarMoeda(2, "", ".", ",");
                    }
        ...
        dataSource: {
            ...
            aggregate: [
                { field: "Valor", aggregate: "sum" },
                { field: "MontanteMI", aggregate: "sum" },
                { field: "Diferenca", aggregate: "sum" }
            ]
        },
        columns: [
            { field: "ContaContabil", title: "Conta Contábil", width: "120px" },
            ...
            { field: "DataLancamento", title: "Data Lançamento", footerTemplate: '<div><span style=\'float:right;\'>TOTAL GERAL:</span></div>', template: "#= kendo.toString(kendo.parseDate(DataLancamento, 'yyyy-MM-dd'), 'dd/MM/yyyy') #", width: "120px" },
            { field: "DocNum", title: "Nº Documento", width: "120px" },
            { field: "Valor", width: "120px", title: "Valor", template: '#=Valor.FormatarMoeda(2, "", ".", ",") #', attributes: { style: "text-align:right;" }, footerTemplate: "<div style='float: right'>#= kendo.toString(sum, 'c2') #</div>" },
            { field: "MontanteMI", width: "120px", title: "Montante MI", template: '#=MontanteMI.FormatarMoeda(2, "", ".", ",") #', attributes: { style: "text-align:right;" }, footerTemplate: "<div style='float: right'>#= kendo.toString(sum, 'c2') #</div>" },
            { field: "Diferenca", width: "120px", title: "Diferença", template: '#=Diferenca.FormatarMoeda(2, "", ".", ",") #', attributes: { style: "text-align:right;" }, footerTemplate: "<div style='float: right'>#= kendo.toString(sum, 'c2') #</div>" }
        ]
    });
}
