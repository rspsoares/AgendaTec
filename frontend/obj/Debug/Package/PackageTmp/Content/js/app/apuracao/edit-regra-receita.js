function BuscarDadosGridEdit(id, idVigencia) {
    $("#loading-page").show();
    $("#gridEdit").html("");
    ...
    $("#gridEdit").kendoGrid({
        ...
        dataSource: {
            transport: {
                read: {
                    url: "/Apuracao/GetRuleParameters?Id=" + id + "&IdVigencia=" + idVigencia,
                    type: "GET",
                    dataType: "json",
                    cache: false,
                    async: false
...
//function GetDsGridEdit(id, idVigencia) {
    ...
    if (obj.length > 0) {
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res.Sucesso)
                    ShowModalSucess(res.Msg);
                else
                    SShowModalAlertres.Msg);
        ...
        SShowModalAlert'Para excluir é necessário selecionar as condições desejadas.');
    }
}