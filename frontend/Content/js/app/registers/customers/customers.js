function PageSetup() {
    ...
    $('#btnClear').click(function () {
        ...
        $("#switchStatus").bootstrapSwitch('state', true);
        LoadCustomers();
    });
    ...
    $('#dtDataContratacao').kendoDatePicker({
        format: "dd/MM/yyyy",
        start: "day"
    });

    $('#chkAtivo').bootstrapSwitch();
...
function LoadCustomers() {
    ...
    $("#grid").kendoGrid({
        ...
        columns: [
            ...
            { field: "RazaoSocial", title: "Razao Social", width: "60%" },
            { field: "DataContratacao", title: "Data Contratação", template: "#= kendo.toString(kendo.parseDate(DataContratacao, 'yyyy-MM-dd'), 'dd/MM/yyyy') #", width: "15%" },
            { field: "Ativo", title: "Ativo", width: "15%", template: "#:StatusDescription(Ativo)#"},            
            {
                title: " ",
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.Sucess) {
                ...
                $("#txtRazaoSocial").val(result.Data.RazaoSocial);
                $("#txtCPFCNPJ").val(result.Data.CNPJ_CPF);
                $("#txtEndereco").val(result.Data.Endereco);
                $("#txtTelefone").val(result.Data.Telefone);
                $("#dtDataContratacao").val(kendo.toString(kendo.parseDate(result.Data.DataContratacao, 'yyyy-MM-dd'), 'dd/MM/yyyy'));
                $('#chkAtivo').prop('checked', result.Data.Ativo);
                $("#txtObservacao").val(result.Data.Observacao);
            }
            else {
                ShowModalAlerta(result.errorMessage);
                return;
            }
        }
    });
...
function SaveCustomer() {
    ...
    customer = {
        ...
        CNPJ_CPF: $("#txtCPFCNPJ").val(),
        Telefone: $("#txtTelefone").val(),
        Endereco: $("#txtEndereco").val(),
        DataContratacao: kendo.parseDate($("#dtDataContratacao").val(), "dd/MM/yyyy"),
        Ativo: $('#chkAtivo').bootstrapSwitch('state'),
    ...
    $.ajax({
        ...
        success: function (result) {
            $("#loading-page").hide();
            if (result.Sucess) {
                ShowModalSucesso("Gravação concluída com sucesso.");    
                $('#modalCustomerEdit').modal("hide");
                LoadCustomers();
            }                
            else                 
                ShowModalAlerta("Erro ao gravar alterações.");            
        }
    });
}
