...
function BuscarDadosGrid(obj) {
    ...
    $("#grid").kendoGrid({
        toolbar: ["excel"],
        excel: {
            fileName: 'RegrasReceita_' + Math.floor((1 + Math.random()) * 0x10000).toString(16).substring(1) + '.xlsx',
            allPages: true
        },
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.Sucesso) {
                    ...
                    ShowModalAlerta("Erro: " + result.Msg);
            }
        });
    });
}
...
function EditRegra(e) {
    ...
    $('#txtNaturezaReceita').val(regra.NaturezaCodigo.trim());
    ...
    $('#modalViewReceita').modal({ backdrop: 'static', keyboard: false });
    $('#modalViewReceita').modal('show');    
}

function ShowPopupVigenciaReceita() {
    var dataFinal;
    ...
    $('#modalVigenciaReceita .modal-dialog .modal-header center .modal-title strong').html("");
    $('#modalVigenciaReceita .modal-dialog .modal-header center .modal-title strong').html("Vigência regra: " + $('#txtDescTipoMosaic').val());
    $('#modalVigenciaReceita').modal({ backdrop: 'static', keyboard: false });
    $('#modalVigenciaReceita').modal('show');
}

function CriarNovaVigenciaReceita() {
    var msgErro = CriarNovaVigencia();

    if (msgErro == '') {
        $('#modalVigenciaReceita').data('data', null);
        $('#modalVigenciaReceita').modal("hide");
...
//function ShowPopupVigenciaReceitaLog() {
//    CarregarGridVigenciaLog(); 

//    $('#modalVigenciaReceitaLog .modal-dialog .modal-header center .modal-title strong').html("");
//    $('#modalVigenciaReceitaLog .modal-dialog .modal-header center .modal-title strong').html("Histórico de vigências da regra: " + $('#txtDescTipoMosaic').val());
//    $('#modalVigenciaReceitaLog').modal({ backdrop: 'static', keyboard: false });
//    $('#modalVigenciaReceitaLog').modal('show');
//}