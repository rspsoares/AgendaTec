...
function AdicionarRequisicao() {
    ...
    if (ddlPeriodo.value() == 0) {
        ShowModalAlerta('Um período deve ser selecionado.');
        return;
    }
    ...
    if (ddlTipo.value() == 0) {
        ShowModalAlerta('O Tipo da Exportação deve ser selecionado.');
        return;
    }
    ...
    if (nomeTarefa == "") {
        ShowModalAlerta('O Nome da Tarefa deve ser preenchido.');
        return;
    }
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.Sucesso)
                ShowModalSucesso('Tarefa adicionada.');
            else
                ShowModalAlerta('Falha na adição de Tarefa.');
        }
    });
