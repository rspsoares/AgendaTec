(function ($) {
    var options = {}; function init(plot) {
        var secondPass = false; var defaultPadding = 2; plot.hooks.draw.push(function (plot, ctx) {
            if (!secondPass) {
                $.each(plot.getAxes(), function (axisName, axis) {
                    var opts = axis.options || plot.getOptions()[axisName]; if (!opts || !opts.axisLabel)
                        return; var w, h; var padding = opts.axisLabelPadding === undefined ? defaultPadding : opts.axisLabelPadding; if (opts.axisLabelUseCanvas != false)
                            opts.axisLabelUseCanvas = true; if (opts.axisLabelUseCanvas) {
                                if (!opts.axisLabelFontSizePixels)
                                    opts.axisLabelFontSizePixels = 14; if (!opts.axisLabelFontFamily)
                                        opts.axisLabelFontFamily = 'sans-serif'; w = opts.axisLabelFontSizePixels + padding; h = opts.axisLabelFontSizePixels + padding;
                            } else { var elem = $('<div class="axisLabels" style="position:absolute;">' + opts.axisLabel + '</div>'); plot.getPlaceholder().append(elem); w = elem.outerWidth(true); h = elem.outerHeight(true); elem.remove(); }
                    if (axisName.charAt(0) == 'x')
                        axis.labelHeight += h; else
                        axis.labelWidth += w; opts.labelHeight = axis.labelHeight; opts.labelWidth = axis.labelWidth;
                }); secondPass = true; plot.setupGrid(); plot.draw();
            } else {
                $.each(plot.getAxes(), function (axisName, axis) {
                    var opts = axis.options || plot.getOptions()[axisName]; if (!opts || !opts.axisLabel)
                        return; if (opts.axisLabelUseCanvas) {
                            var ctx = plot.getCanvas().getContext('2d'); ctx.save(); ctx.font = opts.axisLabelFontSizePixels + 'px ' +
